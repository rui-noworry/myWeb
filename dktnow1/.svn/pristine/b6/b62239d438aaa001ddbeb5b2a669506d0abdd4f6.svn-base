...
class ApplyClassAction extends CommonAction{
    ...
    public function pass() {
        ...
        if (!$res = M('Class')->where($data)->getField('c_id')) {
            $data['c_created'] = time();
            $res = M('Class')->add($data);
        }
        M('ApplyClass')->where(array('ac_id' => $id))->save(array('ac_is_pass' => $res));
        M('ApplyAuth')->where(array('ac_id' => $id))->save(array('c_id' => $res));
        echo $res;
    }
