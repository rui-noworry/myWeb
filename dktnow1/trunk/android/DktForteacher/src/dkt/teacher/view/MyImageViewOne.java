...
import dkt.teacher.model.NoteForHan;
...
public class MyImageViewOne extends ImageView{
	...
	public MyImageViewOne(Context context, NoteForHan note, Bitmap bm, IsDrawBg isDrawBg) {
		super(context);
		this.note = note;
		this.bitmap = bm;
...
//	    };
	...
	public MyImageViewOne(Context context,AttributeSet attr) {
		super(context, attr);
...
//	    		removeCallbacks(mLongPressRunnable); 
	...
	private void setScaleForOneBottom(float temp,int flag) {      
	    ...
	    note.setImgX(getLeft());
		note.setImgY(getTop());
		note.setImgHeight(getHeight());
		note.setImgWidth(getWidth());
	}   
	/**  
	 * 实现处理缩放  
	 */   
