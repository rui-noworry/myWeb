...
import dkt.teacher.activity.NoteListActivity;
import dkt.teacher.activity.GraffitiActivity;
import dkt.teacher.activity.CourseActivity;
import dkt.teacher.activity.HomeActivity;
import dkt.teacher.activity.ResourceActivity;
...
import dkt.teacher.util.StringUtil;
import dkt.teacher.util.UpdateApp;
import dkt.teacher.util.ViewUtil;
import dkt.teacher.view.popu.SetIpPopu;
import dkt.teacher.view.popu.VesionPopu;
...
public class DktForteacherActivity extends Activity {
	...
	Intent intent = new Intent("dkt.teacher.socket.SocketPushService");
    ...
    private void initView() {
    	context = DktForteacherActivity.this;
...
//				mHandler.sendEmptyMessage(DOWN_IMAGE_NO);
    ...
    private void doGetIndexImg() {
		...
		new HttpApacheMapPostThread(temp, new IndexHandler(DktForteacherActivity.this,
...
//			dismissDialog(MyContants.HTTP_WAITING);
    	...
    	findViewById(R.id.main_myresourse_btn).setOnClickListener(
			new OnClickListener() {
				...
				public void onClick(View v) {
					...
					Intent intent  = new Intent(DktForteacherActivity.this,ResourceActivity.class);
    	...
    	findViewById(R.id.main_graffiti_btn).setOnClickListener(
			new OnClickListener() {
				...
				public void onClick(View v) {
					...
					Intent intent  = new Intent(DktForteacherActivity.this, GraffitiActivity.class);
					startActivity(intent);
				}
		});
    	...
    	findViewById(R.id.main_note_btn).setOnClickListener(
			new OnClickListener() {
				...
				public void onClick(View v) {
					...
					Intent intent = new Intent(DktForteacherActivity.this,
    	...
    	findViewById(R.id.main_user_out_btn).setOnClickListener(
			new OnClickListener() {
				...
				public void onClick(View v) {
					...
					AlertDialog.Builder dialog=new AlertDialog.Builder(DktForteacherActivity.this);
    ...
    private void doLoginSucess(String result) {
    	if(result.indexOf("errCode") > 0) {
    		try {
    			...
    			if(errCode1.equals("4")) {
    				ViewUtil.myToast(context, "非教师帐号登录");
    				return;
    	...
    	try {
			...
			if(isServiceRunning("dkt.teacher.socket.SocketPushService")) {
...
//				context.stopService(intent);
			...
			// 跳转到课程页面
			Intent intent  = new Intent(DktForteacherActivity.this, CourseActivity.class);
			startActivity(intent);
