...
// 通过学校类型获取年级
function getGradeByType($type, $s_id = 0, $grade_id = 0) {
    ...
    if ($s_id) {
        ...
        $res = explode(',', $school[$s_id]['s_type']);
    } else {
        $res = array_flip(C('SCHOOL_TYPE'));
        sort($res);
    }
...
// 获取课程封面
function getCourseCover($fileName = '', $type = 0, $num = 210, $url = 1) {
...
// 智慧豆
function addBean($a_id, $s_id, $c_id, $obj_type, $obj_id, $b_num, $b_status = 1, $b_action = 0) {

    if (!$a_id || !$s_id || !$c_id || !$obj_type || !$obj_id) {
        return ;
    }
...
/**
 * showNameByDirectoryCode
 * 根据课文标签码显示名称
 *
 * @param string $code 标签码 三位
 * @param int $num 位置
 */
function showNameByDirectoryCode($code, $num) {

    if ($num) {

        $position = intval($num) - 1;

        switch ($position){
            case 0:
                $showTxt = getTypeNameById($code{$position}, 'SCHOOL_TYPE');
                break;
            case 1:
                $showTxt = getGradeByType($code{0}, '', $code{$position});
                break;
            case 2:
                $showTxt = getTypeNameById($code{$position}, 'SEMESTER_TYPE');
                break;
        }
    } else {

        $showTxt = getTypeNameById($code{0}, 'SCHOOL_TYPE') . getGradeByType($code{0}, $code{1}) . getTypeNameById($code{2}, 'SEMESTER_TYPE');
    }
...
// 获取资源图片
function getResourceImg($res, $type = 0, $size = '100', $url = 1) {
