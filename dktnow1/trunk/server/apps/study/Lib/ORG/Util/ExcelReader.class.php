...
define('ExcelReader_BIFF8',             0x600);
define('ExcelReader_BIFF7',             0x500);
define('ExcelReader_WORKBOOKGLOBALS',   0x5);
define('ExcelReader_WORKSHEET',         0x10);

define('ExcelReader_TYPE_BOF',          0x809);
define('ExcelReader_TYPE_EOF',          0x0a);
define('ExcelReader_TYPE_BOUNDSHEET',   0x85);
define('ExcelReader_TYPE_DIMENSION',    0x200);
define('ExcelReader_TYPE_ROW',          0x208);
define('ExcelReader_TYPE_DBCELL',       0xd7);
define('ExcelReader_TYPE_FILEPASS',     0x2f);
define('ExcelReader_TYPE_NOTE',         0x1c);
define('ExcelReader_TYPE_TXO',          0x1b6);
define('ExcelReader_TYPE_RK',           0x7e);
define('ExcelReader_TYPE_RK2',          0x27e);
define('ExcelReader_TYPE_MULRK',        0xbd);
define('ExcelReader_TYPE_MULBLANK',     0xbe);
define('ExcelReader_TYPE_INDEX',        0x20b);
define('ExcelReader_TYPE_SST',          0xfc);
define('ExcelReader_TYPE_EXTSST',       0xff);
define('ExcelReader_TYPE_CONTINUE',     0x3c);
define('ExcelReader_TYPE_LABEL',        0x204);
define('ExcelReader_TYPE_LABELSST',     0xfd);
define('ExcelReader_TYPE_NUMBER',       0x203);
define('ExcelReader_TYPE_NAME',         0x18);
define('ExcelReader_TYPE_ARRAY',        0x221);
define('ExcelReader_TYPE_STRING',       0x207);
define('ExcelReader_TYPE_FORMULA',      0x406);
define('ExcelReader_TYPE_FORMULA2',     0x6);
define('ExcelReader_TYPE_FORMAT',       0x41e);
define('ExcelReader_TYPE_XF',           0xe0);
define('ExcelReader_TYPE_BOOLERR',      0x205);
define('ExcelReader_TYPE_UNKNOWN',      0xffff);
define('ExcelReader_TYPE_NINETEENFOUR', 0x22);
define('ExcelReader_TYPE_MERGEDCELLS',  0xE5);

define('ExcelReader_UTCOFFSETDAYS' ,    25569);
define('ExcelReader_UTCOFFSETDAYS1904', 24107);
define('ExcelReader_MSINADAY',          86400);
//define('ExcelReader_MSINADAY', 24 * 60 * 60);

//define('ExcelReader_DEF_NUM_FORMAT', "%.2f");
define('ExcelReader_DEF_NUM_FORMAT',    "%s");
...
class ExcelReader {
     ...
     * @access private
     */
    var $_defaultFormat = ExcelReader_DEF_NUM_FORMAT;
    ...
    function _parse() {
        ...
        if (($version != ExcelReader_BIFF8) &&
            ($version != ExcelReader_BIFF7)) {
            return false;
        }

        if ($substreamType != ExcelReader_WORKBOOKGLOBALS){
            return false;
        }
        ...
        while ($code != ExcelReader_TYPE_EOF) {
            switch ($code) {
                case ExcelReader_TYPE_SST:
                    //echo "Type_SST\n";
                     $spos = $pos + 4;
    ...
    //                                              echo "Str $i = $retstr\n";
                ...
                case ExcelReader_TYPE_FILEPASS:
                    return false;
                    break;
                case ExcelReader_TYPE_NAME:
                    //echo "Type_NAME\n";
                    break;
                case ExcelReader_TYPE_FORMAT:
                ...
                case ExcelReader_TYPE_XF:
                        ...
                        //echo "\n";
                    break;
                case ExcelReader_TYPE_NINETEENFOUR:
                    //echo "Type.NINETEENFOUR\n";
                    $this->nineteenFour = (ord($this->data[$pos+4]) == 1);
                    break;
                case ExcelReader_TYPE_BOUNDSHEET:
                    //echo "Type.BOUNDSHEET\n";
                        ...
                        }elseif ($version == ExcelReader_BIFF7){
    ...
    function _parsesheet($spos)
    {
        ...
        if (($version != ExcelReader_BIFF8) && ($version != ExcelReader_BIFF7)) {
            return -1;
        }

        if ($substreamType != ExcelReader_WORKSHEET){
            return -2;
        }
        $spos += $length + 4;
        while($cont) {
            $lowcode = ord($this->data[$spos]);
            if ($lowcode == ExcelReader_TYPE_EOF) break;
            ...
            switch ($code) {
                case ExcelReader_TYPE_DIMENSION:
                    //echo 'Type_DIMENSION ';
                    if (!isset($this->numRows)) {
                        if (($length == 10) ||  ($version == ExcelReader_BIFF7)){
                    ...
                    break;
                case ExcelReader_TYPE_MERGEDCELLS:
                    ...
                    break;
                case ExcelReader_TYPE_RK:
                case ExcelReader_TYPE_RK2:
                    //echo 'ExcelReader_TYPE_RK'."\n";
                    ...
                    break;
                case ExcelReader_TYPE_LABELSST:
                        $row        = ord($this->data[$spos]) | ord($this->data[$spos+1])<<8;
                        $column     = ord($this->data[$spos+2]) | ord($this->data[$spos+3])<<8;
            ...
            //var_dump($this->sst);
                ...
                case ExcelReader_TYPE_NUMBER:
                    ...
                    break;
                case ExcelReader_TYPE_FORMULA:
                case ExcelReader_TYPE_FORMULA2:
                    $row    = ord($this->data[$spos]) | ord($this->data[$spos+1])<<8;
                    $column = ord($this->data[$spos+2]) | ord($this->data[$spos+3])<<8;
                ...
                case ExcelReader_TYPE_BOOLERR:
                    ...
                    //echo 'Type_BOOLERR '."\n";
                    break;
                case ExcelReaderh_TYPE_ROW:
                case ExcelReader_TYPE_DBCELL:
                case ExcelReader_TYPE_MULBLANK:
                    break;
                case ExcelReader_TYPE_LABEL:
                    $row    = ord($this->data[$spos]) | ord($this->data[$spos+1])<<8;
                    $column = ord($this->data[$spos+2]) | ord($this->data[$spos+3])<<8;
    ...
    {
        if ($numValue > 1) {
            $utcDays = $numValue - ($this->nineteenFour ? ExcelReader_UTCOFFSETDAYS1904 : ExcelReader_UTCOFFSETDAYS);
            $utcValue = round(($utcDays+1) * ExcelReader_MSINADAY);
            ...
            $secs = floor($numValue * ExcelReader_MSINADAY) - $hours * 60 * 60 - $mins * 60;
