...
class StudentAction extends CommonAction{
    ...
    public function edit() {
        ...
        foreach ($class as $key => $value) {
            ...
            $classInfo[$key]['c_name'] = replaceClassTitle($value['s_id'], $value['c_type'], $c_grade, $value['c_title'], 0, $value['ma_id']);
    ...
    public function delete() {
        ...
        // 删除班级学生数据
        M('ClassStudent')->where(array('a_id' => array('IN', $ids), 's_id' => $this->authInfo['s_id']))->delete();

        // 更新学生在校日志
        $data_school['as_end_time'] = time();
        $data_school['as_status'] = 5;

        M('AuthSchool')->where(array('a_id' => array('IN', $ids), 's_id' => $this->authInfo['s_id']))->save($data_school);

        $result['status'] = 1;

        // 学生所在班级人数减1
        $classStudent = M('ClassStudent')->where(array('a_id' => array('IN', $ids), 's_id' => $this->authInfo['s_id']))->select();

        $cids = getValueByField($classStudent, 'c_id');

        foreach ($cids as $cvalue) {
            M('Class')->where(array('c_id' => $cvalue, 's_id' => $this->authInfo['s_id']))->setDec('c_peoples');
        }
