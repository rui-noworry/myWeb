        ...
        switch ($name) {
            ...
            case 'model':
                ...
                foreach ($list as $key => $val){
                    ...
                    $val['attrs'] = M('Attribute')->where($map)->field('at_id,at_name,at_title')->select();
                    $val['attrs'] = setArrayByField($val['attrs'], 'at_id');
 ...
 */
function getListByPage($table, $order, $where = array(), $num = 10, $ajax = 0, $p = "", $param = false) {
 ...
 * 返回所有父类数组 $type = 2
 */
function getResourceCategoryParents($id, $s_id = 0, $type = 1, $resource = array(), $return = array()) {
    ...
    if (!$resource) {
        $resource = M('ResourceCategory')->where(array('s_id' => $s_id))->select();
        $resource = setArrayByField($resource, 'rc_id');
    }
    ...
    if ($res) {
        ...
        if ($type == 1) {
            $return[$type] = '<a href="__URL__/index/rc_pid/'.$res['rc_pid'].'">' . $res['rc_title'] . '</a>' . $return[$type];
        }

        if ($type == 2) {
            $return[$type][$res['rc_id']] = $res;
        }

        return getResourceCategoryParents($res['rc_pid'], $s_id, $type, $resource, $return);
...
// 转mp4
function videoToMp4($file_path, $template, $save_path='', $flag = TRUE){
...
function trans($type = 0, $ids = array(), $template = array()) {
    ...
    foreach ($transData as $key => $value) {
        ...
        if (in_array(strtolower($value[$ext]), $docType)) {
            ...
            $mp4Name = videoToMp4($filePath, $sample, $dir);
...
// 资源下载奖励
function resourceDownloadAward($config = array(), $limit) {

    // 读取配置
    if (!$config) {
        $config = C('RESOURCE_DOWNLOAD_AWARD');
        $start = 0;
        $end = 10;
    } else {
        $start = 1 + $limit;
    ...
    // 查询达到要求但没有奖励分数的资源
    $res = M('Resource')->where(array('re_downloads' => array('EGT', array_search(current($config), $config))))->field('re_id,re_downloads,re_downloads_award,a_id,re_title')->limit($start, $end)->select();
    ...
    foreach ($res as $key => $value) {
        ...
        // 如果下载次数和上次的下载次数不一致，则继续
        if ($value['re_downloads'] != $value['re_downloads_award']) {

            // 处理分数
            $points = processPoints($config, $value['re_downloads'], $value['re_downloads_award']);

            // 如果有分数，则更新资源上传人的积分
            if ($points) {
                M('Auth')->where(array('a_id' => $value['a_id']))->setInc('a_points', $points);

                // 发送消息给该用户
                $a_type = M('Auth')->where(array('a_id' => $value['a_id']))->field('a_type,a_is_manager')->find();
                ...
                M('Message')->add(array('me_identity' => $me_identity, 'me_content' => '恭喜你，由于你上传的资源' . $value['re_title'] . '下载量达到' . $value['re_downloads'] . ',系统奖励了你' . $points . '积分。希望你再接再厉，上传更多优质资源！', 'me_a_id' => 0, 'a_id' => $value['a_id'], 'me_created' => time()));
            }

            // 更新资源的最新的下载次数
            M('Resource')->where(array('re_id' => $value['re_id']))->save(array('re_downloads_award' => $value['re_downloads']));
        }
    }

    // 递归
    resourceDownloadAward($config, $limit);

}

// 处理分数
function processPoints($arr, $re_downloads, $re_downloads_award) {

    // 使用count存储分数
    $count = 0;

    // 首次写入，则累加分数
    if ($re_downloads_award == 0) {
        foreach ($arr as $key => $value) {
            if ($key <= $re_downloads) {
                $count += $value;
            }
