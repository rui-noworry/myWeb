...
class BaseAction extends Action {
    ...
    public function _initialize() {
        ...
        if (!in_array(strtolower(MODULE_NAME), C('ALLOW_ACTION'))) {
            ...
            $this->myapps = myapps($authInfo['c_id'], $authInfo['a_applications'], $authInfo['a_is_manager']);
        ...
        if ($authInfo['s_id']) {
            ...
            if (strpos($school[$authInfo['s_id']]['s_apps'], '2') === FALSE) {
                $this->error('您所在的学校未开通资源平台');
            }

            if (strpos($school[$authInfo['s_id']]['s_apps'], '1') !== FALSE) {
                $this->studyOn = C('STUDY_URL');
            }
        } else {
            $this->studyOn = C('STUDY_URL');
        }

        if (!file_exists(C('HTML_PATH') . 'new.txt')) {
            D('CronTab')->autoCache();
        }
    }
