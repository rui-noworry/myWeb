...
//            unorderStyle = { 'disc' : /^[l\u00B7\u2002]/, 'circle' : /^[\u006F\u00D8]/,'square' : /^[\u006E\u25C6]/},
    ...
    function upload_localimg(img_path) {
        var obj = new ActiveXObject("Mink.FileTransfer");
        var result = obj.Upload('http://' + window.location.host + '/Index/upload', img_path);
        ...
        switch ( node.tag ) {
            ...
            case 'img':
                ...
                if ( node.attributes.src && /^(?:file)/.test( node.attributes.src ) ) {
                    ...
                    {
                        var obj = new ActiveXObject("Mink.FileTransfer");
                        var localimage_path = node.attributes.word_img.substr(8);
                        var result = upload_localimg(localimage_path);
                        node.attributes.src = result;
                    } catch(e){
