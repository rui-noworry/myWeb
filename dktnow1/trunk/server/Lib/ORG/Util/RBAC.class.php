...
class RBAC extends Think
{
    ...
    static function saveAccessList($name='_ACCESS_LIST',$authId=null)
    {
        if(null===$authId)   $authId = $_SESSION[C('USER_AUTH_KEY')];
        // 如果使用普通权限模式，保存当前用户的访问权限列表
        // 对管理员开发所有权限
        if(C('USER_AUTH_TYPE') !=2 && !$_SESSION[C('ADMIN_AUTH_KEY')] )
            $_SESSION[$name]    =    RBAC::getAccessList($authId);
        return ;
    }
    ...
    // 登录检查
    static public function checkLogin($name='_ACCESS_LIST') {
        //检查当前操作是否需要认证
        if(RBAC::checkAccess()) {
            //检查认证识别号
    ...
    //权限认证的过滤器方法
    static public function AccessDecision($appName=APP_NAME,$name='_ACCESS_LIST')
    {
        //检查是否需要认证
        if(RBAC::checkAccess()) {
            //存在认证识别号，则进行进一步的访问决策
    ...
    static public function getAccessList($authId)
    {
        ...
        $sql = "SELECT node.n_id,node.n_name FROM ".
                    $table['role']." AS role,".
                    $table['user']." AS user,".
                    $table['access']." AS access ,".
                    $table['node']." AS node ".
                    "WHERE user.u_id='{$authId}' AND user.r_id=role.r_id AND ( access.r_id=role.r_id  OR (access.r_id=role.r_pid AND role.r_pid!=0 ) ) AND role.r_status=1 AND access.n_id=node.n_id AND node.n_level=1 AND node.n_status=1";
        $apps = $db->query($sql);
        ...
        foreach($apps as $key=>$app) {
            ...
            $sql = "SELECT node.n_id,node.n_name FROM ".
                    ...
                    $table['access']." AS access ,".
                    $table['node']." AS node ".
                    "WHERE user.u_id='{$authId}' AND user.r_id=role.r_id AND ( access.r_id=role.r_id  OR (access.r_id=role.r_pid AND role.r_pid!=0 ) ) AND role.r_status=1 AND access.n_id=node.n_id AND node.n_level=2 AND node.n_pid={$appId} AND node.n_status=1";
            ...
            foreach($modules as $key=>$module) {
                ...
                $moduleId = $module['n_id'];
                $moduleName = $module['n_name'];
                ...
                if('PUBLIC'== strtoupper($moduleName)) {
                    ...
                    $table['access']." AS access ,".
                    $table['node']." AS node ".
                    "WHERE user.u_id='{$authId}' AND user.r_id=role.r_id AND ( access.r_id=role.r_id  OR (access.r_id=role.r_pid AND role.r_pid!=0 ) ) AND role.r_status=1 AND access.n_id=node.n_id AND node.n_level=3 AND node.n_pid={$moduleId} AND node.n_status=1";

                    $rs = $db->query($sql);

                    foreach ($rs as $a){
                        $publicAction[$a['n_name']] = $a['n_id'];
                    }
